...
TFL (Top for LLM) — это легковесный консольный монитор (TUI), созданный для отслеживания использования ресурсов (CPU, RAM, GPU) и метрик локально запущенных языковых моделей (LLM) через `llama.cpp` и другие бэкенды. 
...
- 📊 **Мониторинг процессов DRM (Linux):** Точное отображение процессов, использующих VRAM.
